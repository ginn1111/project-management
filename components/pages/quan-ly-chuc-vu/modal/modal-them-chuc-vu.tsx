...
import { removeSign } from '@/utils/helpers';
...
const ModalThemChucVu = (props: IModalThemChucVu<Partial<IPosition>>) => {
	...
	const { setValue, handleSubmit, register, getValues, reset } = useForm({
		...
		<Modal {...rest} loading={isLoading}>
			...
			>
					...
					<Input
						{...register('name', {
							onChange: (e) => {
								setValue(
									'code',
									removeSign(e.target.value?.toUpperCase())?.replaceAll(
										/\s/g,
										'_'
									)
								);
				...
				<div>
					...
					<Input {...register('code')} disabled placeholder="code" />
				</div>
